...
import { MicPermissionHelp } from "@/components/common";
...
const upFile = (file: any) => {
  ...
  reader.onload = (e) => {
    ...
    // 立即显示图片
    st.value.fileBase64.push(base64Data);
    ...
    if (!canVisionModel(gptConfigStore.myData.model)) {
      upImg(file)
        ...
        .catch((e) => {
          // 上传失败，移除本地预览
          const index = st.value.fileBase64.indexOf(base64Data);
          if (index > -1) {
            st.value.fileBase64.splice(index, 1);
          }
          ms.error(e);
        });
    } else {
  ...
  rec
    ...
    .setOnError((error: string) => {
      ...
      ms.error(error, {
        ...
        }, { default: () => t('mj.micPermissionHelpTitle') || 'Microphone Help' })
      });
      st.value.micStart = false;
    })
    .setOpt({
...
const getFileExtension = (url: string): string => {
  ...
  return dotIndex > -1 ? filename.substring(dotIndex + 1).toUpperCase() : 'FILE';
};

// 获取文件名
const getFileName = (url: string, index: number): string => {
  if (!url) return `文件 ${index + 1}`;
  try {
    const parts = url.split('/');
    const filename = parts[parts.length - 1];
    // 如果文件名太长，截取显示
...
<template>
  ...
  <div v-else>
    ...
    >
      <div
        class="w-[80px] h-[80px] rounded-lg bg-slate-50 mr-2 mt-2 relative group overflow-hidden border-2 border-dashed border-gray-200 hover:border-blue-300 transition-all duration-200"
      ...
      >
        ...
        <!-- 删除按钮 -->
        <div
          class="absolute top-1 right-1 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 cursor-pointer hover:bg-red-600"
          @click="st.fileBase64.splice(st.fileBase64.indexOf(v), 1)"
        >
          <SvgIcon icon="mdi:close" class="text-xs" />
        </div>
      </div>
    </div>
