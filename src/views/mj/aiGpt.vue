...
watch(()=>homeStore.myData.act, async (n)=>{
    ...
    if(n=='gpt.submit' ||  n=='gpt.whisper'  ){
        ...
        if( dd.fileBase64 && dd.fileBase64.length>0 ){
            ...
            if( !canVisionModel(model)  ) model='Qwen/Qwen3-VL-30B-A3B-Instruct';
            // 设置是否有附件为true
        ...
        if( n=='gpt.whisper'){
            ...
            try{
                ...
                promptMsg.model='FunAudioLLM/SenseVoiceSmall';
        ...
        if( dd.fileBase64 && dd.fileBase64.length>0 ){
            if(isCanBase64Model(model)){ 
                ...
                if(nGptStore.base64ImageMode){
                    // 如果启用了base64模式，将图片URL转换为base64格式
                    for(let f of dd.fileBase64){
                        if(f.indexOf('http')>-1){
                            try{
                                const { url2base64 } = await import("@/api/mjsave");
                                const base64Result = await url2base64(f);
                                obj.content.push({ "type": "image_url",  "image_url": {url: base64Result.base64 }   });
                                mlog('图片URL已转换为base64格式', f);
                            }catch(e){
                                mlog('图片URL转换base64失败', e);
                                // 转换失败时使用原URL
       ...
       controller.value && controller.value.abort();
        ...
        //return ;
        if(['FunAudioLLM/SenseVoiceSmall','midjourney'].indexOf(model)>-1){
        ...
        subTTS({model:'fnlp/MOSS-TTSD-v0.5',input: actData.text , 
        // voice:nGptStore.tts_voice
         }).then(d=>{
...
const submit= (model:string, message:any[],opt?:any)=>{
    ...
    controller.value = new AbortController();
        if(model=='FunAudioLLM/SenseVoiceSmall'){
            ...
            //mlog('FunAudioLLM/SenseVoiceSmall2323',opt  );
            const formData = new FormData( );
