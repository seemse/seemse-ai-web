...
export function upImg(file: any): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
        ...
        fetch(getUrl('/resource/oss/upload'), {
            method: 'POST',
            headers: header,
            body: formData
        })
...
const getUrl=(url:string)=>{
    ...
    return `${window.APP_CONFIG?.VITE_GLOB_API_URL || '/api'}${url}`;
}
...
export const isCanBase64Model=(model:string)=>{
    //gpt-4o
    //customVisionModel
    let arr=['Qwen/Qwen3-VL-30B-A3B-Instruct','gpt-image','gpt-4o','gemini','1.5','sonnet','opus'];
    for( let m of arr){
...
export const isTTS= ( model:string )=>{
    if(model.indexOf('fnlp/MOSS-TTSD-v0.5')===0 )return true;
    return false ;
}
