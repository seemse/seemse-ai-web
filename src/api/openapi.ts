...
const getUrl = (url: string) => {
	...
	// 使用全局配置的 VITE_GLOB_API_URL
	const baseUrl = window.APP_CONFIG?.VITE_GLOB_API_URL || "/api";
	return `${baseUrl}${url}`;
};
...
export const GptUploader = (_url: string, FormData: FormData) => {
	...
	if (uploadType == "r2") {
		...
		let url = gptGetUrl(`/openapi${_url}`);
		return uploadNomalDo(url, headers);
...
export const subModel = async (opt: subModelType) => {
	...
	// 处理消息中的图片URL，将HTTP地址转换为base64格式
	if (opt.message && Array.isArray(opt.message)) {
		opt.message = await Promise.all(
			opt.message.map(async (msg) => {
					...
					msg.content = await Promise.all(
						msg.content.map(async (content) => {
							if (
								...
								// 检查是否是本地文件URL（http://192.168.x.x 格式的内网地址）
								if (
									url.startsWith("http://192.168.") ||
									url.startsWith("https://192.168.")
								) {
									try {
										// 使用url2base64函数将HTTP图片转换为base64
										const { url2base64 } = await import("@/api/mjsave");
										const result = await url2base64(url);
										if (result && result.base64) {
											content.image_url.url = result.base64;
										}
									} catch (error) {
	...
	let body = {
		...
		stream: true,
		kid: gptConfigStore.myData.kid || '1980441662755254274',
		// kid: "1980441662755254274",
		// kid: "",
			...
			onMessage: async (data: string) => {
				...
				if (data == "[DONE]") opt.onMessage({ text: "", isFinish: true });
					...
					try {
						...
						const hasErrorShape =
							obj && (obj.error || obj.code || obj.reason || obj.message);
						...
						if (
							/FAILED_TO_AUTH|unauthorized|未认证|认证失败|invalid\s*api\s*key/i.test(
								s
							)
						) {
...
export const getHistoryMessage = async (
	...
	// 读取当前会话配置，判断是否启用 base64 图片模式
	let chatConfig: any = null;
	try {
		const uuid = dataSources.find((ds) => ds.uuid)?.uuid;
		if (uuid) {
			const chatSet = new chatSetting(uuid);
			chatConfig = chatSet.getGptConfig();
		}
	} catch (e) {
		mlog("获取会话配置失败", e);
	}
		...
		try {
			if (typeof txt === "string") {
				...
				if (s.startsWith("{") && s.endsWith("}")) {
					...
					if (o && ("code" in o || "reason" in o || "message" in o))
						return true;
				}
				if (
					/失败|错误|FAILED|fail|unauthorized|未认证|未找到模型|模型选择/.test(
						s
					)
				)
					return true;
	...
	// 若启用 base64 图片模式，将图片 URL 转为 base64
	const convertImageUrlsToBase64 = async (content: any) => {
		if (!chatConfig?.base64ImageMode) return content;
		if (Array.isArray(content)) {
			...
			for (const item of content) {
				if (item.type === "image_url" && item.image_url?.url) {
					try {
						const { url2base64 } = await import("@/api/mjsave");
						const base64Result = await url2base64(item.image_url.url);
						converted.push({
							...item,
							image_url: { url: base64Result.base64 },
						});
						mlog("图片 URL 已转换为 base64", item.image_url.url);
					} catch (err) {
						mlog("图片 URL 转换 base64 失败", err);
						converted.push(item);
					}
				} else {
					converted.push(item);
	...
	for (let ii = istart; ii >= 0; ii--) {
		...
		// 若启用 base64 图片模式，转换用户消息中的图片 URL
		if (chatConfig?.base64ImageMode && o.inversion) {
			try {
				content = await convertImageUrlsToBase64(content);
			} catch (err) {
				mlog("转换消息内容失败", err);
			}
		}
